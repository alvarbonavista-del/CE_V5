"""Reparto de suscripciones entre conexiones WebSocket de OKX. PURO, SIN IO.

Limites PUBLICADOS por OKX (verificados contra la doc vigente, no de memoria):
- 240 suscripciones de canal por conexion (error 60014 al pasarse). Usamos 200 de
  margen: preferimos fallar nosotros, en claro, antes que que OKX rechace.
- 3 peticiones de conexion por segundo por IP; 480 subscribe/unsubscribe por hora y
  conexion. Ese ritmo lo respeta el connector, no este planificador.
- OKX NO publica un tope de conexiones concurrentes por IP como Binance (300/IP/5min):
  max_connections es un techo PROPIO conservador, no un limite de OKX.
"""
...
class OkxLimits:
    """Limites de OKX (spot), con margen propio."""

    # 240 es el limite publicado (error 60014); 200 es margen a proposito.
    max_subscriptions_per_connection: int = 200
    # OKX no publica tope de conexiones concurrentes: techo propio conservador.
...
class ConnectionPlanner:
    ...
    donde esta. Solo se reubica si su conexion desaparece. Sin esto, un alta podria
    recolocar las demas: cerrar y reabrir todas es una tormenta de reconexiones y un
    hueco de datos en cada una. El coste de un alta debe ser proporcional al alta.
    """

    def __init__(self, limits: OkxLimits = DEFAULT_LIMITS) -> None:
        self._limits = limits
