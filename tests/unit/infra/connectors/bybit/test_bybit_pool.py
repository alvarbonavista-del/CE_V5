"""Pruebas del planificador de conexiones de Bybit (T-03). Hermeticas, sin IO."""

from __future__ import annotations

import pytest

from ce_v5.infra.connectors.bybit.pool import (
    BybitLimits,
    ConnectionPlanner,
...
def test_reparte_dentro_de_una_conexion() -> None:
    limits = BybitLimits(max_subscriptions_per_connection=3, max_connections=2)
...
def test_desborda_a_la_segunda_conexion() -> None:
    limits = BybitLimits(max_subscriptions_per_connection=2, max_connections=2)
    plan = ConnectionPlanner(limits).assign({"a", "b", "c"})
    assert len(plan) == 2
...
def test_estabilidad_una_nueva_no_recoloca_las_viejas() -> None:
    limits = BybitLimits(max_subscriptions_per_connection=2, max_connections=5)
...
def test_pasarse_de_capacidad_no_abre_nada() -> None:
    limits = BybitLimits(max_subscriptions_per_connection=1, max_connections=1)
