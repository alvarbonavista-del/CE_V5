"""Doble en memoria del puerto EventBus (ADR-013), compartido por los tests.

Vive AQUI y no en un conftest porque lo necesitan tanto los tests unitarios como el
test de EQUIVALENCIA de integracion (CA-12 punto 5), que compara lo que dice este
doble con lo que dice el motor real. Un conftest solo es visible para su propio
arbol, y esa frontera habria obligado a duplicar el doble: dos dobles que se
separan en silencio son peores que ninguno.

Es soporte de TEST: jamas se cablea en la aplicacion.
"""
...
class InMemoryEventBus:
    ...
    def latest_offset(self, topic: str) -> Offset | None:
        """El offset de la ULTIMA entrada, o None si el topic esta vacio.

        Semantica EQUIVALENTE a la del adapter Redis: mismo resultado en las mismas
        condiciones (la cadena del offset es especifica de cada backend; lo que coincide
        es la POSICION que designa).
        """
        log = self._logs.get(topic, [])
        if not log:
            return None
        return Offset(str(log[-1].seq))
