"""Validacion en caliente REAL de P07b 3a-ii-Bybit: trades de Bybit -> hechos.

Hermana de validate_binance/okx_trades_live.py, para Bybit. Conecta el WebSocket REAL de
Bybit (topic 'publicTrade' en la conexion publica spot, COMPARTIDA con las velas) y
convierte TRADES REALES individuales en filas de market_trade contra la base LOCAL de
juguete, con la maquinaria de la 3a-i (TradeIngestionEngine + PostgresTradeWriter) y el
conector de la 3a-ii-Bybit.

Graduada del scratch tools/probe_bybit_trades_live.py (el sondeo previo): aquel confirmo
que 'i'/'execId' son enteros contiguos del MISMO espacio y que recent-trade capa a 60
sin paginar; esta prueba el CAMINO ENTERO.
...
asi que el multiplexado (velas y trades por la misma conexion, enrutados por prefijo de
topic) y el relleno REST solo se validan aqui. Lo que el CI cubre a fondo es la
traduccion (dos: WS y REST) y la cobertura puras.

LO PROPIO DE BYBIT FRENTE A OKX/BINANCE:
- Topic 'publicTrade.<native>' en la conexion spot; ping {"op":"ping"} <20 s SIEMPRE.
- Lado agresor = campo 'S' (Buy/Sell) -> buy/sell; WS y REST usan NOMBRES distintos.
- Relleno recent-trade de UNA sola llamada (NO pagina) y capado a 60 EN SILENCIO: la
  ventana es pequena, asi que un corte de mas de ~60 trades queda INCOMPLETO. En Bybit
  el fail-safe (covered=False) es el camino COMUN, no la excepcion.
- Simbolo PEGADO (BTCUSDT): set_symbol_map resuelve native<->canonical, como en velas.
...
Uso: python tools/validate_bybit_trades_live.py
...
from ce_v5.infra.connectors.bybit.connector import (  # noqa: E402
    _REST_TRADES_MAX,
    BybitSpotConnector,
)
...
# trades de LOS DOS lados. En un par ilnquido el reparto podria salir a un solo lado.
...
# Umbrales HOLGADOS: BTC-USDT spot en Bybit es de BAJA CADENCIA (el sondeo vio ~1/s).
...
def _fallar_red(contexto: str, exc: Exception) -> NoReturn:
    ...
    print(
        "NO es un fallo del codigo de la 3a-ii-Bybit: es alcanzabilidad de red. Si "
        "Bybit no se alcanza desde esta maquina, se dice y se para.",
        file=sys.stderr,
    )
    raise SystemExit(1)
...
def _rest_backfill(
    connector: BybitSpotConnector, last_seen: LastSeenTrade
) -> TradeBackfillResult:
    """El relleno REST por el MISMO camino del motor (recent-trade, una llamada)."""
    return connector.backfill_after_reconnect(_CLAVE, last_seen)


def _sonda(connector: BybitSpotConnector) -> Sequence[RawTrade]:
    """ANTES de abrir el socket: confirma que el REST responde. Si no, aborta.

    Devuelve la ventana REST (hasta 60 trades, el techo real), que ademas ancla el check
    del hueco: su trade mas antiguo es un punto que el mercado dejara atras.
    """
    print("=== FASE 0: sonda REST previa (no abre WebSocket) ===")
    print(
        f"  GET /v5/market/recent-trade?category=spot&symbol=BTCUSDT&limit="
        f"{_REST_TRADES_MAX} ..."
    )
    try:
        # Primera conexion: sin nada persistido, una llamada; no hay hueco que cubrir.
        ventana = _rest_backfill(
            connector, LastSeenTrade(trade_id=None, event_time_ms=None)
        ).raw_trades
    except (OSError, json.JSONDecodeError) as exc:
        _fallar_red("recent-trade (GET /v5/market/recent-trade)", exc)

    if not ventana:
        print(
            "FALLO: recent-trade respondio pero SIN trades para BTCUSDT. Inesperado en "
            "un par tan liquido: revisar antes de abrir el socket.",
            file=sys.stderr,
        )
        raise SystemExit(1)
    ...
    trade = ventana[0]
    print(
        f"  [OK] Bybit alcanzable. Ventana REST: {len(ventana)} trades. "
...
def _check_agresor(reader_db: Database) -> bool:
    ...
    print(
        ...
        "delataria un bug traduciendo el campo 'S' de Bybit."
        if ok
        else f"  {marca} falta un lado: 'S' NO se traduce bien (o el mercado no dio "
...
def _check_reconexion(
    ...
    """(c) Reconexion CORTA: el hueco (< 60 trades) queda cubierto y el solape dedupa.

    Un corte breve deja un hueco pequeno que la ventana de 60 de recent-trade SI cubre:
    el motor rebootstrapea solo, covered=True, no se apunta ningun hueco.
    """
    print("\n=== COMPROBACION (c): reconexion CORTA (hueco < 60) -> cubierta ===")
    ...
    marca = "[OK]" if ok else "[FALLO]"
    print(
        f"  {marca} reconexion real ({reconn}); el motor rebootstrapeo SOLO ({boot} "
        f"trades por REST); el solape se dedupo ({dup}); el hueco corto quedo CUBIERTO "
        "(sin huecos nuevos)"
    )
    return ok
...
def _check_hueco_mayor_que_la_ventana(connector: BybitSpotConnector) -> bool:
    """(c-largo) Un corte de mas de ~60 trades se marca INCOMPLETO (fail-safe).

    EN BYBIT ESTE ES EL CAMINO COMUN: recent-trade solo da 60 trades y no pagina, asi
    que cualquier corte mas largo que eso deja dato que el REST ya no alcanza. Lo unico
    honesto es covered=False, para que 3b marque las barras afectadas.

    COMO SE PROVOCA SIN ESPERAR: Bybit numera los trades con un id ENTERO y CONTIGUO por
    instrumento (verificado en el sondeo), asi que un id muy anterior al actual es un
    trade REAL que existio. Se pide el relleno con un last_seen cuyo id esta
    DELIBERADAMENTE mas atras de lo que 60 trades alcanzan: el relleno (una llamada, 60)
    no llega y declara el hueco. Es el camino de produccion, contra el REST REAL.
    """
    print("\n=== COMPROBACION (c-largo): hueco > 60 -> INCOMPLETO (fail-safe) ===")
    ...
    # Mas atras de lo que 60 trades alcanzan, con amplio margen: covered=False seguro.
    id_sintetico = id_actual - 10 * _REST_TRADES_MAX
    print(
        f"  id actual (REST): {id_actual}; last_seen sintetico (real y contiguo): "
        f"{id_sintetico} -- {10 * _REST_TRADES_MAX} trades atras, > la ventana de "
        f"{_REST_TRADES_MAX}"
    )
    ...
    marca = "[OK]" if ok else "[FALLO]"
    print(
        f"  trades del relleno={len(resultado.raw_trades)} (<= {_REST_TRADES_MAX}); "
        f"covered={resultado.covered}; hueco=[{resultado.gap_from_event_time_ms}, "
        f"{resultado.gap_to_event_time_ms}]"
    )
    print(
        f"  {marca} el relleno NO alcanzo el last_seen y el hueco quedo DECLARADO "
        "(covered=False): en Bybit el fail-safe es lo normal, no un fallo."
    )
    return ok
...
def main() -> None:
    ...
    try:
        print(
            "Validacion en caliente REAL de trades de Bybit (P07b 3a-ii-Bybit). "
            "Sandbox local, feed publico, sin credenciales."
        )
        print(
            f"  parametros: ventana={ventana_s:.0f}s objetivo={objetivo} trades "
            f"techo REST del relleno={_REST_TRADES_MAX} (una llamada, no pagina)\n"
        )

        # FASE 0: sonda REST. Si Bybit no responde, se para AQUI, sin abrir socket.
        _sonda(connector)
        ...
        # FASE 1: catalogo (puebla set_symbol_map: Bybit pega el simbolo) + stream.
        ...
        if not stream_ok:
            ...
            print(f"  stream abierto: {_CLAVE_STR} (topic publicTrade.BTCUSDT)")
            _drenar_durante(
                engine,
                observing,
                ventana_s,
            ...
            if engine.metrics.trades_persisted == 0:
                ...
                checks["c) reconexion corta cubierta"] = _check_reconexion(
                    connector, engine, observing, migrations_db
                )
                checks["c-largo) hueco > 60 -> fail-safe"] = (
                    _check_hueco_mayor_que_la_ventana(connector)
                )
    ...
    if not (stream_ok and checks and all(checks.values())):
        print(
            "\nVALIDACION EN CALIENTE 3a-ii-Bybit (trades): FALLIDA. Una validacion "
            "que miente es peor que ninguna.",
            file=sys.stderr,
        )
        raise SystemExit(1)
    print(
        "\nVALIDACION EN CALIENTE 3a-ii-Bybit (trades): OK. Trades reales de Bybit -> "
        "hechos persistidos, con lado agresor exacto, dedup, reconexion cubierta, "
        "fail-safe frecuente si el hueco excede los 60, y reproducibilidad."
    )
