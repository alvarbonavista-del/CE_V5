"""Validacion en caliente REAL de OKX (T-03): streaming, reconexion, bootstrap, dedup.

Conecta el WebSocket REAL de OKX, recibe velas REALES y las convierte en hechos del
sistema contra la base LOCAL de juguete. Demuestra lo que un fake NO puede: velas vivas
por el socket y la deduplicacion del bootstrap REST tras una reconexion. Es la MISMA
maquinaria de P07 (motor, dedup, bootstrap) validando un exchange NUEVO: parte del
veredicto de CE-14. El connector se construye POR EL REGISTRO (resolve("okx")).
...
GUARDIA 5.20: un solo proceso porta varios roles (app, migraciones, ingesta) porque esta
validacion exige el sistema entero. Cada cargador ve el sub-entorno con SOLO su DSN
(_solo). Limpiar velas exige el rol OWNER (migraciones): market_candle es append-only.

Uso: python tools/validate_okx_live.py
...
_EMAIL = "hot-t03-okx-live@ejemplo.test"
...
def _exigir_env() -> None:
    ...
    if faltan:
        print(
            ...
            f"OKX: {', '.join(faltan)}. Exige el sistema completo (app + migraciones + "
...
class _ObservingSource:
    ...
    vela vista en poll(), para imprimir un precio real y vivo. No cambia el
...
class _CatalogoEnIngesta:
    ...
    permite ese rol (regla 5.20). Satisface CatalogWriterPort (upsert + deactivate).
    """
...
def main() -> None:
    ...
    # El connector se construye POR EL REGISTRO: resolve("okx"). El isinstance da acceso
    # a las primitivas de operacion (force_reconnect_all, shutdown, metrics), que no
    # estan en el puerto. Que la misma maquinaria resuelva OKX es parte del veredicto.
    connector = build_default_registry().resolve("okx")
    if not isinstance(connector, OkxSpotConnector):
        msg = "el registro no devolvio un OkxSpotConnector para kind='okx'."
    ...
    try:
        print("Validacion en caliente REAL de OKX (T-03). Sandbox local.")
        ...
        # OKX: el instId ya es canonico, asi que native_symbol == symbol (identidad).
        catalogo.upsert("okx", "spot", "BTC-USDT", "BTC-USDT", "active")
        _sembrar_intent(scoped_db, tenant_id, user_id, clock)
        print(f"  preparado: usuario {_EMAIL}, tenant {tenant_id}, interes en BTC-USDT")

        # sync_catalog con el connector REAL: upsert al catalogo. OKX NO implementa
        # SymbolMapSink (resuelve el simbolo por identidad), asi que aqui no hay
        # set_symbol_map: la deteccion estructural de sync_catalog lo salta solo.
    ...
    if not (stream_ok and fase2_ok):
        print(
            "\nVALIDACION EN CALIENTE OKX: FALLIDA. Una validacion que miente es peor "
            "que ninguna.",
            file=sys.stderr,
        )
        raise SystemExit(1)
    print("\nVALIDACION EN CALIENTE OKX: OK. Streaming, reconexion y dedup, vivos.")
