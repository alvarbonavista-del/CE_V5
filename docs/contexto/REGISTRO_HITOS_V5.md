...
ci_local 24/24). 7 factores activos en pivotphase.confidence (formula_version 4, techo
100), gate estructural de fase 3 VIVO, notrade destopado a 100 con el bloque L2. M3
SIGUE ABIERTO en 7 de 8: falta P09a).
...
de 8: faltan P09a y la reapertura de P08c).
...
| M3 | Una Rule dispara sobre datos reales y proyecta signal.*/alert.*; el router backend entrega por un canal no-PWA/mock (sin overlay, sin ejecucion) | P07, P07b, P07c, P08, P08b, P08c, P08c-reapertura, P09a | ABIERTO (7 de 8: P07, P07b, P07c, P08, P08c, P08b y la REAPERTURA de P08c ENTREGADAS; falta P09a; EXPANDIDO por EXP-M3-01) |
...
- P08b ENTREGADA -- DataSources candle-derived. Firmada Alvaro 2026-08-06. PR #6, Actions VERDE 3/3
  ...
  Detectores desbloqueados colocados en la REAPERTURA de P08c (ya cerrada, ver entrada
  siguiente). Recomendaciones CSA: formula_version al activar F1-F7, tabla viva F1-F7,
  documentar patron ScalarValue.
- P08c REAPERTURA CERRADA -- detectores footprint + completar el modelo de confianza.
  2026-08-06, merge 4ec79f7 en main, 11 commits (ff9da47 a 36d2a4a). Cierra los diferidos
  que P08b desbloqueo.
  Entregables: candle.high/low POINT_LOCAL; los CUATRO detectores WINDOWED
  (absorption/climax/void/notrade, 10 fuentes servibles); imbalance.buy_stack/sell_stack
  (F5, POINT_LOCAL); activacion de F1/F3/F5/F7 en pivotphase.confidence.
  MODELO COMPLETO: los SIETE factores activos a 1/7, formula_version 4, techo efectivo
  100 (peso E input vivo en los siete). Cero factores diferidos.
  GATE ESTRUCTURAL DE FASE 3 VIVO: AbsorptionZone armada desde close + lado de absorcion,
  invalidacion PHASE3_ZONE_BREAK y 11o parametro phase3_break_threshold. El skip
  autorizado de phase3_zone_break, arrastrado desde el cierre de P08c, queda RESUELTO
  (0 skips propios en la suite).
  NOTRADE DESTOPADO: bloque L2 vivo (4 features sobre el frontier del libro, grant 0029);
  el score pasa de tope 65 a 100 y la banda 'toxic' (>=80) deja de ser inalcanzable.
  Dictamenes: P08c-DET-01, P08c-CONF-01 a P08c-CONF-05.
  EXCEPCION EXC-CI-P08C-01: el merge se apoya en ci_local 24/24 porque la CI de GitHub
  Actions estuvo caida por infraestructura (7 intentos; ci.yml IDENTICO al que paso en
  P08b, diff vacio; el fallo era asignacion de runner, del lado de GitHub).
  Ver REGISTRO_DECISIONES seccion 39.
  M3: 7 de 8; SIGUE ABIERTO (falta P09a).
